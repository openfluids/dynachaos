...
def test_on_off_intermittency_compute_writes_golden_cache(tmp_path):
    ...
    payload = on_off_intermittency_figure.compute(output_path)
    ...
    np.testing.assert_array_less(-1.7, payload["off_time_alpha"])
...
def test_on_off_intermittency_plot_writes_png(tmp_path):
    payload = on_off_intermittency_figure.compute(None)
