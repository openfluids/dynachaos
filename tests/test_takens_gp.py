...
def _lorenz(n=N):
    ...
    return solve_ivp(f, (0, te[-1]), [1., 1., 1.], t_eval=te, method="RK45",
                     rtol=1e-9, atol=1e-11).y[0, 5000:]
...
def _vdp_wave(n, dt=0.05, mu=2.0):
    ...
    return te, solve_ivp(f, (0, te[-1]), [1., 0.], t_eval=te, method="RK45",
                         rtol=1e-9, atol=1e-11).y[0]
...
def _vdp_cycle(n=N):
    ...
    return solve_ivp(f, (0, te[-1]), [2., 0.], t_eval=te, method="RK45",
                     rtol=1e-9, atol=1e-11).y[0, 4000:]
...
def _tt(sig, tau, m, w):
    d, *_ = takens_theiler_dimension(_embed(_norm(sig), m, tau), n_r=60,
                                     max_pairs=2_000_000, theiler_window=w)
    return d


# -------- direct Takens-Theiler dimension recovery (generous regression bands) --
@pytest.mark.parametrize("name, sig, tau, m, w, lo, hi", [
...
def test_protocol_defers_noise():
    ...
    r = gp_dimension_robust(np.random.default_rng(0).standard_normal(N),
                            m_max=10, n_segments=6)
    assert r["gp_certifiable"] is False
