...
|---:|---|---:|---|---|---|
| 1 | Streaming RQA run scans (`rqa_streaming_from_trajectory`) | 0.729482 s, 107.3 MB at N=4000 explicit eps; dense RQA has an 8*N^2 byte distance envelope and 4 GiB threshold near N=23170 in `scale_envelope.md` | High: enables long-signal RQA without dense recurrence matrices | Medium: exact diagonal + vertical counting, metrics/theiler edge cases | Next new Rust kernel if porting starts |
