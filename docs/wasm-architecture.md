...
| `rust/wasm` | `wasm-bindgen` exports over `rust/core`; `rotation_number_tile` for the picture, `rotation_number_point` for the quoted readout, `zero_one_k` for the 0-1 test for chaos, and `correlation_counts`, `apen_counts`, `fuzzy_entropy_sum`, `ordinal_distribution`, `diagonal_lines`, `vertical_lines`, `multifractal_moments`, `ami_histogram` and `select_dimension_cao` for the diagnostics panel | `dynachaos-core` with `--no-default-features` |
